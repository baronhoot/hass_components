...
LUTRON_MODEL_TO_TYPE = {
    ...
    "HQRD-HN6BRL": "SeeTouchHybridKeypad",
    "HQR-T15CRL-XX": "SeeTouchTabletopKeypad",
...
SEETOUCH_KEYPAD_6_BUTTON_TYPES_TO_LEAP = {
    "button_1": 1,
    "button_2": 2,
    "button_3": 3,
    "button_4": 4,
...
SEETOUCH_KEYPAD_6_BUTTON_TRIGGER_SCHEMA = LUTRON_BUTTON_TRIGGER_SCHEMA.extend(
    {
        vol.Required(CONF_SUBTYPE): vol.In(
            SEETOUCH_KEYPAD_6_BUTTON_TYPES_TO_LEAP
        ),
...
BUTTON_DEVICE_EVENT_TYPES = {"pressed", "released"}
BUTTON_DEVICE_TRIGGER_SCHEMA = LUTRON_BUTTON_TRIGGER_SCHEMA.extend(
    {
        vol.Required(CONF_TYPE): vol.In(BUTTON_DEVICE_EVENT_TYPES),
...
DEVICE_TYPE_SCHEMA_MAP = {
    ...
    "SeeTouchHybridKeypad": BUTTON_DEVICE_TRIGGER_SCHEMA, 
    "SeeTouchTabletopKeypad": BUTTON_DEVICE_TRIGGER_SCHEMA,
    "HomeownerKeypad": BUTTON_DEVICE_TRIGGER_SCHEMA,
    "Pico3ButtonRaiseLower": BUTTON_DEVICE_TRIGGER_SCHEMA,
}
...
DEVICE_TYPE_SUBTYPE_MAP_TO_LEAP = {
    ...
    "SeeTouchHybridKeypad": SEETOUCH_KEYPAD_6_BUTTON_TYPES_TO_LEAP,
    "SeeTouchTabletopKeypad": SEETOUCH_KEYPAD_6_BUTTON_TYPES_TO_LEAP,
    "HomeownerKeypad": SEETOUCH_KEYPAD_6_BUTTON_TYPES_TO_LEAP,
    "Pico3ButtonRaiseLower": PICO_3_BUTTON_RAISE_LOWER_BUTTON_TYPES_TO_LEAP,
}
...
TRIGGER_SCHEMA = vol.Any(
    ...
    SEETOUCH_KEYPAD_6_BUTTON_TRIGGER_SCHEMA,
    BUTTON_DEVICE_TRIGGER_SCHEMA
)
...
async def async_validate_trigger_config(
    ...
    _LOGGER.debug(f"async_validate_trigger_config: {device}, config={config}")
...
async def async_get_triggers(
    ...
    if device_type in QSX_KEYPADS:
        # look up buttons from entity registry
        entity_registry = er.async_get(hass)
        device_ents = er.async_entries_for_device(entity_registry, device_id)

        for trigger in [ACTION_PRESSED, ACTION_RELEASED]:
            for entity in device_ents:
                triggers.append(
                    {
                        ...
                        CONF_DOMAIN: DOMAIN,
                        CONF_TYPE: trigger,
                        CONF_SUBTYPE: entity.original_name,
                        CONF_ENTITY_ID: entity.entity_id
                    }
                )
    else:
        valid_buttons = DEVICE_TYPE_SUBTYPE_MAP_TO_LEAP.get(device_type, {})
...
async def async_attach_trigger(
    ...
    _LOGGER.debug(f"async_attach_trigger device:{device} device_type:{device_type}, serial:{serial}, schema:{schema}")

    if device_type in QSX_KEYPADS:
        _LOGGER.debug(f"attaching trigger for QSX Keypad {serial}: {config}")
        event_config = {
        ...
        _LOGGER.debug(f"attaching trigger for non-QSX Keypad {serial}: {config}")
