"""Valve representing the shutoff valve for the Flo by Moen integration."""
...
async def async_setup_entry(
    ...
    """Set up the Flo valves from config entry."""
    ...
    for device in devices:
        if device.device_type != "puck_oem":
            entities.append(FloValve(device))
...
class FloValve(FloEntity, ValveEntity):
    """Valve class for the Flo by Moen valve."""
    ...
    def __init__(self, device: FloDeviceDataUpdateCoordinator) -> None:
        """Initialize the Flo valve."""
        super().__init__("shutoff_valve", device)
        self._attr_is_closed = device.last_known_valve_state != "open"
    ...
    def async_update_state(self) -> None:
        ...
        self._attr_is_closed = self._device.last_known_valve_state != "open"
        self.async_write_ha_state()
